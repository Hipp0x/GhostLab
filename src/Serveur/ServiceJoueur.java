...
public class ServiceJoueur implements Runnable {
    ...
    public boolean optionsInGame(OutputStream os, InputStream is, boolean exit ) throws IOException {
        String[] infos;
        int gameId;
        Partie p;
        while(true){
            ...
            switch (action) {
                ...
                case "SIZE?":
                    ...
                    p = findGame(gameId);
                    if(p != null){
                        os.write(("SIZE! "+ gameId + " " + game.getLaby().getH() + " " + game.getLaby().getW() + "***").getBytes(),0,16);
                        os.flush();
                    }else{
                        dunno(os);
                    }
        ...
        do {
            ...
            switch (action) {
                ...
                case "SIZE?":
                    ...
                    p = findGame(gameId);
                    if (p != null) {
                        os.write(("SIZE! " + gameId + " " + p.getLaby().getH() + " " + p.getLaby().getW() + "***").getBytes(), 0, 16);
                    }else{
                        dunno(os);
                    }
                    clearIS(is);
